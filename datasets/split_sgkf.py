...
def get_class_bmi(meta_df, img_paths):
    ...
    y = [meta_df[meta_df['ID']==int(os.path.dirname(fname).split('ID')[-1])]['bmi_desc'].values[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_json_path', type=str, default='/opt/ml/data/train_split.json',
                        help='Path to save json file containing train filenames.')
    parser.add_argument('--valid_json_path', type=str, default='/opt/ml/data/valid_split.json',
                        help='Path to save json file containing validation filenames.')
    parser.add_argument('--class_by', type=str, default='bmi', choices=['bmi'],
