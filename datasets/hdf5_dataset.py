...
class Hdf5Dataset(Dataset):
    def __init__(self, config, is_train=True, transforms=None):
        ...
        if self.is_train:
            if self.config.validation:
                ...
                self.train_filename = glob(os.path.join(self.config.train_hdf5_data_dir, "*.h5py"))[0]
                self.valid_filename = glob(os.path.join(self.config.valid_hdf5_data_dir, "*.h5py"))[0]
        else:
    ...
    def __len__(self):
        if self.config.validation:
            ...
            total_length = 0
            with h5py.File(self.train_filename, "r") as hf:
                train_num = len(hf.keys())
                self.train_num = train_num
                total_length += train_num
            with h5py.File(self.valid_filename, "r") as hf:
                valid_num = len(hf.keys())
                self.valid_num = valid_num
                total_length += valid_num
            return total_length
    ...
    def __getitem__(self, idx):
        if self.config.validation:
            ...
            if idx < self.train_num:
                self.file = h5py.File(self.train_filename, "r")
            else:
                self.file = h5py.File(self.valid_filename, "r")
                    ...
                    for ann in target_annotations:
                        ...
                        if target_c == 19 or target_c == 20 or target_c == 25 or target_c == 26:
                            ...
                            while x in bone_area_x:
                                x = random.randint(int(100 * scale),int(1800 * scale))
                            x -= alpha      
