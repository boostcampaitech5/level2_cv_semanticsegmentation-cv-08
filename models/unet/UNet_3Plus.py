...
from torchvision.models import resnet152, resnet50, resnet34
import numpy as np
...
class UNet_3Plus(nn.Module):
    def __init__(
        self, n_classes=1, in_channels=3, feature_scale=4, is_deconv=True, is_batchnorm=True
    ):
...
class UNet_3Plus_ResNet152(nn.Module):

    def __init__(self, n_classes=1, in_channels=3, feature_scale=4, is_deconv=True, is_batchnorm=True):
        super(UNet_3Plus_ResNet152, self).__init__()
        self.is_deconv = is_deconv
        self.in_channels = in_channels
        self.is_batchnorm = is_batchnorm
        self.feature_scale = feature_scale
        self.encoder_backbone = resnet152(pretrained=True)
        ...
        self.block1 = nn.Sequential(
            ...
            self.encoder_backbone.bn1,
            self.encoder_backbone.relu
        )
        
        self.block2 = nn.Sequential(
            self.encoder_backbone.maxpool,
            self.encoder_backbone.layer1
        )
        
        self.block3 = self.encoder_backbone.layer2
        self.block4 = self.encoder_backbone.layer3
        self.block5 = self.encoder_backbone.layer4
    ...
    def forward(self, inputs):
        ## -------------Encoder-------------
        h1 = self.block1(inputs)  # h1->320*320*64
...
class UNet_3Plus_DeepSup_CGM(nn.Module):
    def __init__(
        ...
        self.cls = nn.Sequential(
            nn.Dropout(p=0.5), nn.Conv2d(filters[4], 2, 1), nn.AdaptiveMaxPool2d(1), nn.Sigmoid()
        )
