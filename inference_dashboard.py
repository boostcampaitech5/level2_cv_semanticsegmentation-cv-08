...
def load_dataset():
    tf = getattr(augmentations, state.config.test_augmentations.name)(**state.config.test_augmentations.parameters)
...
def visualise():
    for i in range(state.page-1*state.batch_size, state.page*state.batch_size):        
        preds = []
        for rle in state.prediction_cache['rles'][i * len(CLASSES) : i * len(CLASSES) + len(CLASSES)]:
        ...
        filename = class_fname.split('_')[1]
        ...
        with col2:
            st.markdown('<p style="text-align: center;">Segmentation Map</p>',
                        unsafe_allow_html=True)
            st.image(seg_map, use_column_width="auto")

        with col3:
            st.markdown('<p style="text-align: center;">Overlay</p>',
                        unsafe_allow_html=True)
            st.image(overaid_image, use_column_width="auto") 


if __name__ == "__main__":
    st.set_page_config(page_title="Segmentation Inference Dashboard",
                       layout="wide")
